...
1. 업그레이드 제안 전에 ADR/lessons-learned 확인
   ```bash
   grep -r '<package-name>' v2/docs/
   ```

2. 임시 브랜치에서만 시도
   ```bash
   git checkout -b deps-bump-<package>-<version>
   npm install <package>@<version> --workspace v2/client
   ```

3. Full smoke + 브라우저 수동 테스트
   ```bash
   bash v2/skills/dev-server-restart.md  # (재시작 절차)
...
5. 실패 시
   - 롤백: `git checkout -- package.json && rm -rf v2/node_modules/@rhwp v2/client/node_modules/@rhwp && npm install`
...
- [ ] Vite cache 삭제 (`rm -rf v2/*/node_modules/.vite`)
- [ ] dev 서버 재시작
- [ ] 브라우저 하드 리프레시
- [ ] `smoke-test.sh` PASS

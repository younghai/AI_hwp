#!/usr/bin/env bash
# Smoke-test the v2 stack end-to-end.
#
# Pre-reqs: dev server must be running (`cd v2 && npm run dev`).
...
CLIENT_URL="${CLIENT_URL:-http://127.0.0.1:5188}"
SERVER_URL="${SERVER_URL:-http://127.0.0.1:8788}"
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
TEMPLATE="${TEMPLATE:-$REPO_ROOT/templates/gonmun.hwpx}"
...
if [ -z "$DOWNLOAD_URL" ]; then
  ...
  if python3 "$REPO_ROOT/v2/tools/verify-hwpx-markers.py" "$OUT_HWPX" \

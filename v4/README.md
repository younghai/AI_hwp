...
| **자립형 구조** | `v4/scripts/`, `v3/templates/` 내부화 — 외부 경로 의존 0 |
...
```bash
bash v4/hooks/post-deps-change.sh   # 프로세스 kill + Vite cache 삭제
cd v4 && npm run dev                # dev 서버 재시작
# 브라우저에서 Cmd+Shift+R (하드 리프레시)
bash v4/tools/smoke-test.sh         # 정상성 재검증
```

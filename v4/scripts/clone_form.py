#!/usr/bin/env python3
"""
HWPX 양식 복제 도구 (Workflow F) — 독립 CLI/도구 전용

⚠️ 프로덕션 빌드 경로가 아니다. 서버는 build_hwpx.py 만 spawn 하며, 이 스크립트는
R5(단락 정규화/linesegarray 리셋)를 적용하지 않는 별도 문자열-치환 방식이다.
표/이미지/병합셀을 바이트 수준으로 100% 보존해야 하는 수동 복제 시에만 사용한다.
서버 파이프라인에서 재사용하려면 R5 를 준수하도록 재작성해야 한다. (review PY-P2)

...
#
# Usage:
#   bash v4/hooks/post-deps-change.sh
...
pkill -f 'v4/server/index.js' 2>/dev/null && echo "  ✓ express killed" || echo "  · express 미실행"
pkill -f 'concurrently' 2>/dev/null && echo "  ✓ concurrently killed" || echo "  · concurrently 미실행"
sleep 1

echo ""
echo "=== 2. Vite cache 삭제 ==="
for d in "$REPO_ROOT/v4/client/node_modules/.vite" "$REPO_ROOT/v4/node_modules/.vite"; do
  if [ -d "$d" ]; then
    rm -rf "$d"
    echo "  ✓ removed $d"
  else
...
for pj in "$REPO_ROOT/v4/node_modules/@rhwp/core/package.json" "$REPO_ROOT/v4/client/node_modules/@rhwp/core/package.json"; do
  if [ -f "$pj" ]; then
...
cat <<'NEXT'
  ...
  3. 'bash v4/tools/smoke-test.sh' 로 확인
NEXT

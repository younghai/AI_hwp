...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--report-json", help="Optional path to write diagram embedding report JSON")
...
def embed_diagrams(
    ...
    try:
        ...
        for diag_spec in diagrams:
            report["skipped"].append({
                ...
                "afterSection": diag_spec.get("afterSection", ""),
                "afterSectionIndex": diag_spec.get("afterSectionIndex"),
                "reason": f"cairosvg unavailable: {exc}",
            })
        return report

    report["cairosvgAvailable"] = True
    ...
    heading_ids = detect_heading_style_ids(header_path)
    heading_paras: list[ET.Element] = []
    for para in root.iter(f"{{{HP}}}p"):
        if not _paragraph_has_direct_text(para):
            continue
        if para.get("styleIDRef", "0") in heading_ids:
            heading_paras.append(para)

    hpf_tree = _safe_parse(content_hpf)
    hpf_root = hpf_tree.getroot()
    manifest_ns = "http://www.idpf.org/2007/opf/"
    manifest_el = hpf_root.find(f"{{{manifest_ns}}}manifest")
    ...
    for diag_spec in diagrams:
        ...
        if not svg_str:
            ...
            report["skipped"].append({
                ...
                "afterSectionIndex": diag_spec.get("afterSectionIndex"),
                "reason": "render_diagram returned no SVG",
            })
            continue
        ...
        try:
            ...
            report["skipped"].append({
                ...
                "afterSection": after_section,
                "afterSectionIndex": diag_spec.get("afterSectionIndex"),
                "reason": f"svg2png failed: {exc}",
            })
            continue

        bin_counter += 1

        # Find the insertion point: prefer index-based section mapping, then
        # fall back to direct heading text matching for backward compatibility.
        insert_after_para = None
        after_section_index = diag_spec.get("afterSectionIndex")
        if isinstance(after_section_index, int) and 0 <= after_section_index < len(heading_paras):
            insert_after_para = heading_paras[after_section_index]
        elif after_section:
            for para in heading_paras:
                t_nodes = [t for t in para.findall(f".//{{{HP}}}t") if t.text]
                if any(after_section == (t.text or "").strip() for t in t_nodes):
                    insert_after_para = para
                    break
        if insert_after_para is None:
            report["skipped"].append({
                "type": diag_spec.get("type", ""),
                "title": diag_spec.get("title", ""),
                "afterSection": after_section,
                "afterSectionIndex": after_section_index,
                "reason": "section anchor not found",
            })
            if png_path.exists():
                png_path.unlink(missing_ok=True)
            continue

        # Register in content.hpf manifest only after a valid anchor is found.
        if manifest_el is None:
            report["skipped"].append({
                ...
                "afterSection": after_section,
                "afterSectionIndex": after_section_index,
                "reason": "content.hpf manifest not found",
            })
            if png_path.exists():
                png_path.unlink(missing_ok=True)
            continue

        item_el = ET.SubElement(manifest_el, f"{{{manifest_ns}}}item")
...
def load_sections_body(json_path: str | None) -> tuple[dict[str, str] | None, list[dict]]:
    ...
    TOC (see run()) succeed even when the source JSON carries NFD text — which
...
def run() -> Path:
    ...
    if args.report_json:
        ...
        report_path.write_text(json.dumps(diagram_report, ensure_ascii=False, indent=2), encoding="utf-8")

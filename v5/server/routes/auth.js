...
import { requireSession, setSessionProviderSecret } from '../lib/session.js'

export function createAuthRouter({ oauthBase, clientOrigin }) {
  const router = Router()

  router.get('/auth/:provider', requireSession, (req, res) => {
    ...
    rememberState(state, providerKey, req.sessionId)
  ...
  router.get('/auth/:provider/callback', requireSession, async (req, res) => {
    const providerKey = req.params.provider
    const provider = AI_PROVIDERS[providerKey]
    ...
    if (saved.sid !== req.sessionId) {
      return res.send(oauthResultPage(false, '인증을 시작한 사용자 세션과 현재 세션이 일치하지 않습니다.', clientOrigin))
    }
    if (!provider?.oauth) {
    ...
    try {
      ...
      setSessionProviderSecret(req.sessionId, provider.envKey, tokenData.access_token)

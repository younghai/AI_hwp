...
#
# 사용:
#   bash v5/testdata/run-golden.sh                # 모든 케이스 실행
#   bash v5/testdata/run-golden.sh 01-minimal-report  # 특정 케이스만
#
# 요구: dev 서버가 기동 중이어야 함 (`cd v5 && npm run dev`).
# 동작:
...
CASES_DIR="$V5_ROOT/testdata/golden"
...
run_case() {
    ...
    curl -sS -X POST "$SERVER_URL/api/export-hwpx" \
        ...
        -F "sourceFile=@$V5_ROOT/$template" > "$tmp/resp.json"

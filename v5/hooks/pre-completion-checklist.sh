...
#
# Usage:
#   bash v5/hooks/pre-completion-checklist.sh
#
# Exit 0 = 완료 선언해도 OK
# Exit 1 = 회귀/누락 있음 — 사용자에게 실패 보고
#
# v5 는 자립형: 모든 경로가 v5/ 내부로 해결된다.
...
if lsof -nP -i:5194 -i:8794 2>/dev/null | grep -q LISTEN; then
  pass "server 포트 listening"
  echo ""
  echo "=== E2E smoke test (API → HWPX → markers) ==="
  if bash "$V5_ROOT/tools/smoke-test.sh" > /tmp/v5-smoke.log 2>&1; then
    pass "smoke-test.sh PASS"
  else
    fail "smoke-test.sh FAIL — 자세한 로그: /tmp/v5-smoke.log"
    tail -20 /tmp/v5-smoke.log | sed 's/^/      /'
  fi
else
  fail "dev 서버가 꺼져 있음 — E2E 생략. 'cd v5 && npm run dev' 후 재실행."
fi

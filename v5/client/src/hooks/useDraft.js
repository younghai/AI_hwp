...
export function useDraft({ setParseStatus }) {
  ...
  async function generateDraft({ sourceFile, sourceInsight, docType, companyName, goal, notes, targetTitle, docFields, aiProvider, aiModel, aiApiKey, onOptimistic }) {
    if (!sourceFile) {
  ...
  async function buildHwpx({ draftOverride, sourceFile, sourceInsight, docType }) {
    ...
    try {
      ...
      const result = {
        ...
        diagramReport: payload.diagramReport || null
      }

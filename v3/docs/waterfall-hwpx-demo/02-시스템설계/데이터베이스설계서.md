...
| 서버 메모리 (세션) | RAM | 사용자 세션 저장소 (`lib/session.js`), 24h TTL | 비영구 (서버 재시작 시 초기화) |
...
# 서버 설정
PORT=8790
CLIENT_ORIGIN=http://127.0.0.1:5190
OAUTH_REDIRECT_BASE=http://127.0.0.1:8790
...
| 항목 | 설명 |
|------|------|
| 저장소 | 메모리 기반 Map (`server/lib/session.js`) |
| 키 | session ID (crypto.randomBytes 기반) |
| 값 | `{ user: { email, name, picture }, createdAt: number }` |
| TTL | 24시간 (60초 간격 스위프) |
| 쿠키 | `v2_session`, `httpOnly`, `SameSite=Lax`, `Max-Age=86400` |
| 특징 | 서버 재시작 시 초기화 — 데모 환경에 적합 |
